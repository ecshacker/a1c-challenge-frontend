...
export default function Home() {
  ...
  return (
      <div style={{ padding: "40px 20px", background: "#F7F6F2", borderRadius: "8px", marginTop: "40px", boxShadow: "0 4px 6px -1px rgba(0,0,0,0.05)" }}>
        <h1 style={{ fontSize: "24px", fontWeight: 700, marginBottom: "24px" }}>A1C Challenge Instrumentation Router</h1>
        <p style={{ color: "#595C50", marginBottom: "32px", fontSize: "15px", lineHeight: "1.5" }}>
          Select a view below to review rendering and verify UI states. These routes match your file system map directly.
        </p>
        <nav style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {routes.map((route) => (
              <Link
                  ...
                  className="a1c-fade hover:opacity-80"
              >
                {route.name} →
              </Link>
          ))}
